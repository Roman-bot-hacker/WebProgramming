...
function loadDataFromServer() {
    ...
    if (page == "news.html") {
        console.log("start news load");
        var userAction = async () => {
            var response = await fetch('http://127.0.0.1:8000/news');
            var myJson = await response.json();
            console.log(myJson);
            var length = myJson.result.length;
            for (var i=0; i < length; i++) {
                addNews(myJson.result[i].title, myJson.result[i].article);
            }
        }
        userAction();
    }
    else if (page == "fans.html") {
        console.log("start appeals load");
        const userAction = async () => {
            ...
            var length = myJson.result.length;
            for (var i=0; i < length; i++) {
                addAppealFromServer(myJson.result[i].appeal, myJson.result[i].ap_date);
            }
        }
        userAction();
